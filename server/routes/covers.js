...
router.get('/editions', async (req, res) => {
  try {
    const { book_id: rawBookId, isbn: rawIsbn, title: rawTitle, author: rawAuthor } = req.query;
    ...
    const workKey = await resolveOpenLibraryWork({ isbn, title, author });
    if (!workKey) {
      return res.json({ work_key: null, editions: [], candidates_saved: 0 });
    }
    ...
    res.json({ work_key: workKey, editions, candidates_saved: saved });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
async function resolveOpenLibraryWork({ isbn, title, author }) {
  const normalizedIsbn = normalizeIsbn(isbn);
  if (normalizedIsbn) {
...
async function fetchOpenLibraryEditions(workKey, maxRows) {
  const editions = [];
  let offset = 0;
  const pageSize = 50;
